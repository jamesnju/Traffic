...
if (isset($_POST['license_id']) && isset($_POST['driver_password'])) {
	...
	if (empty($license_id)) {
		...
		$sql = "SELECT * FROM driver WHERE driver_license_id='$license_id' AND driver_password='$driver_password'";

		$result = mysqli_query($conn, $sql);

		if (mysqli_num_rows($result) === 1) {
			$row = mysqli_fetch_assoc($result);
			if ($row['driver_license_id'] === $license_id && $row['driver_password'] === $driver_password) {
				$_SESSION['license_id'] = $row['driver_license_id'];
				$_SESSION['driver_name'] = $row['driver_name'];
				$_SESSION['driver_email'] = $row['driver_email'];
				$_SESSION['home_address'] = $row['driver_home_address'];
