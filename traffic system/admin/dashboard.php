...
<body class="overlay-scrollbar">
    ...
    <div class="dashwrapper animated fadeIn">
        <div class="container-fluid">
            <h6 class="mt-1 badge badge-pill badge-light tag-hover" style="padding: 10px; font-size: 0.75rem;">Account Holder : <span><?php echo $_SESSION['registration_username']; ?><span></h6>
...
<?php
    include "../connection.php";
    $year = date("Y");
    $jan = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 01 AND YEAR(issued_fines_date) = '$year'");
    $janVal = mysqli_num_rows($jan);

    $feb = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 02 AND YEAR(issued_fines_date) = '$year'");
    $febVal = mysqli_num_rows($feb);

    $march = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 03 AND YEAR(issued_fines_date) = '$year'");
    $marchVal = mysqli_num_rows($march);

    $april = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 04 AND YEAR(issued_fines_date) = '$year'");
    $aprilVal = mysqli_num_rows($april);

    $may = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 05 AND YEAR(issued_fines_date) = '$year'");
    $mayVal = mysqli_num_rows($may);

    $june = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 06 AND YEAR(issued_fines_date) = '$year'");
    $juneVal = mysqli_num_rows($june);

    $july = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 07 AND YEAR(issued_fines_date) = '$year'");
    $julyVal = mysqli_num_rows($july);

    $august = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 08 AND YEAR(issued_fines_date) = '$year'");
    $augustVal = mysqli_num_rows($august);

    $september = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 09 AND YEAR(issued_fines_date) = '$year'");
    $sepVal = mysqli_num_rows($september);

    $october = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 10 AND YEAR(issued_fines_date) = '$year'");
    $octVal = mysqli_num_rows($october);

    $november = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 11 AND YEAR(issued_fines_date) = '$year'");
    $novVal = mysqli_num_rows($november);

    $december = mysqli_query($con, "SELECT issued_fines_no FROM issued_fines WHERE MONTH(issued_fines_date) = 12 AND YEAR(issued_fines_date) = '$year'");
